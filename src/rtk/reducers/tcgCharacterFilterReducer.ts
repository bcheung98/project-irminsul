...
export const TCGCharacterFilterSlice = createSlice({
    ...
    reducers: {
        setElement: (state, action: PayloadAction<string>) => {
            !state.element.includes(action.payload)
                ? state.element.push(action.payload)
                : state.element.splice(
                      state.element.indexOf(action.payload),
                      1
                  );
        },
        setWeapon: (state, action: PayloadAction<string>) => {
            !state.weapon.includes(action.payload)
                ? state.weapon.push(action.payload)
                : state.weapon.splice(state.weapon.indexOf(action.payload), 1);
        },
        setFaction: (state, action: PayloadAction<string>) => {
            !state.faction.includes(action.payload)
                ? state.faction.push(action.payload)
                : state.faction.splice(
                      state.faction.indexOf(action.payload),
                      1
                  );
        },
        setSortBy: (
            state,
            action: PayloadAction<"name" | "element" | "weapon" | "energy">
        ) => {
...
} = TCGCharacterFilterSlice.actions;
