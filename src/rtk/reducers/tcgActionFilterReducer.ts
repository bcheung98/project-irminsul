    ...
    reducers: {
        setType: (state, action: PayloadAction<string>) => {
            !state.type.includes(action.payload)
                ...
                : state.type.splice(state.type.indexOf(action.payload), 1);
        },
        setSubType: (state, action: PayloadAction<string>) => {
            !state.subType.includes(action.payload)
                ? state.subType.push(action.payload)
                : state.subType.splice(
                      state.subType.indexOf(action.payload),
                      1
                  );
        },
