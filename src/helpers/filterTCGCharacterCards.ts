import { TCGCharacterFilterState } from "reducers/tcgCharacterFilterReducer";
import { TCGCharacterCard, TCGFaction } from "types/tcg";

export const filterTCGCharacterCards = (
    characterCards: TCGCharacterCard[],
    filters: TCGCharacterFilterState,
    searchValue: string
) => {
    ...
    if (filters.faction.length > 0) {
        cards = cards.filter((card) =>
            filters.faction.some((f) => card.factions.includes(f as TCGFaction))
        );
    }
    if (searchValue !== "") {
        cards = cards.filter((card) =>
    ...
    switch (filters.sortBy) {
        ...
        case "energy":
            cards = cards.sort(
                (a: any, b: any) =>
                    b.talents.burst.cost.split(" ")[1].slice(0, -1) >
                        a.talents.burst.cost.split(" ")[1].slice(0, -1) ||
                    a.name.localeCompare(b.name)
            );
            break;
        default:
